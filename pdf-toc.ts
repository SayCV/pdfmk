...
import { default as JSDOM } from 'npm:jsdom@19';
...
export const extractPDFToc = async (pdfBuffer: Buffer, options: Config,
  ...
  const data = await parsePdf(pdfBuffer, { pagerender: pageRender, version: 'v2.0.550' })
  ...
  const document = new JSDOM(content).window.document
  const tocElement: Element | null = document.querySelector('.print-toc')
  ...
  if (tocElement) {
    tocElement.innerHTML = (HandlebarsJS as any).compile(tocTemplate || '')({
      ...context,
      ...tocContext,
    })
